...
import type { Id } from "@reread/convex";

type StatusFilter = "all" | "reading" | "finished" | "want-to-read";

const STATUS_LABELS: Record<string, { label: string; color: string; emoji: string }> = {
  reading: { label: "Reading", color: "#6366f1", emoji: "📖" },
  finished: { label: "Finished", color: "#10b981", emoji: "✅" },
  "want-to-read": { label: "Want to Read", color: "#f59e0b", emoji: "📋" },
};

export default function Home() {
  const [statusFilter, setStatusFilter] = useState<StatusFilter>("all");
  const [selectedUserBookId, setSelectedUserBookId] = useState<Id<"userBooks"> | null>(null);
  ...
  const words = useQuery(
    ...
    selectedUserBookId && showWords ? { userBookId: selectedUserBookId } : "skip"
  );
  ...
  const handleAddWord = useCallback(async () => {
    ...
    try {
      await addWord({
        ...
        pageNumber: wordForm.pageNumber ? parseInt(wordForm.pageNumber) : undefined,
      });
      setWordForm({ word: "", definition: "", context: "", pageNumber: "" });
      setShowAddWord(false);
      Alert.alert("Saved!", `"${wordForm.word}" added.`);
    } catch (error: any) {
      Alert.alert("Error", String(error?.data || error?.message || "Failed to add word"));
    }
  }, [selectedUserBookId, wordForm, addWord]);

  const handleRemoveBook = useCallback(
    async (userBookId: Id<"userBooks">, title: string) => {
      Alert.alert("Remove Book", `Remove "${title}" from your library? This will delete all saved words for this book.`, [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert("Error", String(error?.data || error?.message));
            }
          },
        },
      ]);
  ...
  const handleStatusChange = useCallback(
    async (userBookId: Id<"userBooks">, newStatus: "reading" | "finished" | "want-to-read") => {
      try {
  ...
  const renderBookItem = ({ item }: { item: any }) => {
    ...
    if (!book) return null;
    const status = STATUS_LABELS[userBook.status];
    ...
    return (
      <View style={styles.bookCard}>
        ...
        >
          ...
          <View style={styles.bookInfo}>
            ...
            <View style={styles.metaRow}>
              ...
              <Text style={styles.wordCount}>{wordsCount} word{wordsCount !== 1 ? "s" : ""}</Text>
            </View>
          </View>
        </TouchableOpacity>
        ...
        <View style={styles.bookActions}>
            ...
            {Object.entries(STATUS_LABELS).map(([key, val]) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  userBook.status === key && { backgroundColor: val.color + "30", borderColor: val.color },
                ]}
                onPress={() => handleStatusChange(userBook._id, key as any)}
              >
                <Text style={[styles.miniStatusText, userBook.status === key && { color: val.color }]}>
                  {val.emoji}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <TouchableOpacity onPress={() => handleRemoveBook(userBook._id, book.title)}>
  ...
  return (
    <View style={styles.container}>
        ...
        {(["all", "reading", "finished", "want-to-read"] as const).map((s) => (
          ...
          >
            <Text style={[styles.filterTabText, statusFilter === s && styles.filterTabTextActive]}>
              {s === "all" ? "📚 All" : `${STATUS_LABELS[s].emoji} ${STATUS_LABELS[s].label}`}
            </Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
      ...
      {myBooks === undefined ? (
        ...
        <View style={styles.center}>
          ...
          <Text style={styles.emptyTitle}>
            {statusFilter === "all" ? "No Books Yet" : `No ${STATUS_LABELS[statusFilter]?.label} Books`}
          </Text>
          <Text style={styles.emptySubtitle}>Search for books to add them to your library</Text>
        </View>
      ) : (
        <FlatList
          data={myBooks}
      ...
      {/* Add Word Modal */}
      <Modal visible={showAddWord} transparent animationType="slide" onRequestClose={() => setShowAddWord(false)}>
        <TouchableOpacity style={styles.modalBackdrop} activeOpacity={1} onPress={() => setShowAddWord(false)}>
          <View style={styles.modalContent} onStartShouldSetResponder={() => true}>
            ...
            <TextInput
              ...
              onChangeText={(t) => setWordForm((f) => ({ ...f, definition: t }))}
              multiline
            />
            <TextInput
              style={styles.input}
            ...
            <TextInput
              ...
              onChangeText={(t) => setWordForm((f) => ({ ...f, pageNumber: t }))}
              keyboardType="numeric"
            />

            <TouchableOpacity
              style={[styles.submitBtn, !wordForm.word.trim() && { opacity: 0.5 }]}
            ...
            <TouchableOpacity style={styles.cancelBtn} onPress={() => setShowAddWord(false)}>
      ...
      {/* Words List Modal */}
      <Modal visible={showWords} transparent animationType="slide" onRequestClose={() => setShowWords(false)}>
        <View style={styles.wordsModal}>
          ...
          {words === undefined ? (
            ...
            <View style={styles.center}>
              ...
              <Text style={styles.emptySubtitle}>Tap "+ Word" on a book to start collecting</Text>
            </View>
          ) : (
            <FlatList
              data={words}
...
const styles = StyleSheet.create({
  ...
  noCover: { backgroundColor: "#e5e7eb", justifyContent: "center", alignItems: "center" },
  ...
  emptySubtitle: { fontSize: 14, color: "#6b7280", marginTop: 4, textAlign: "center", paddingHorizontal: 40 },
  modalBackdrop: {
    flex: 1,
  ...
  wordsModal: { flex: 1, backgroundColor: "#fff", marginTop: 60, borderTopLeftRadius: 20, borderTopRightRadius: 20 },
  wordsHeader: {
    flexDirection: "row",
  ...
  wordContext: { fontSize: 13, color: "#6b7280", fontStyle: "italic", marginTop: 4 },
